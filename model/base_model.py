...
class BaseModel():
    ...
    def load_networks(self, which_epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):# and name!='E3':
    ...
    def save_results2(self, save_data, score=None, data_name='none',dir=""):
        """Save the training or testing results to disk"""
        img_paths = self.get_image_paths()
    ...
    def save_uncertainty(self, save_data, score=None, data_name='none'):
            """Save the training or testing results to disk"""
            img_paths = self.get_image_paths()
            ...
            for i in range(save_data.size(0)):
                ...
                name = os.path.splitext(short_path)[0]
                if type(score) == type(None):
                    img_name = '%s_%s.npy' % (name, data_name)
                else:
                    # d_score = score[i].mean()
                    # img_name = '%s_%s_%s.png' % (name, data_name, str(round(d_score.item(), 3)))
                    img_name = '%s_%s_%s.npy' % (name, data_name, str(score))
                #img_name = '%s.png' % (name)
                # save predicted image with discriminator score
                util.mkdir(self.opt.results_dir)
                img_path = os.path.join(self.opt.results_dir, img_name)
                img_numpy = save_data[i].detach().cpu().numpy()
                np.save(img_path,img_numpy)
